...
function parseArgs(args) {
  ...
  for (const arg of args) {
    ...
    else if (arg.startsWith('--concurrency=')) opts.concurrency = Math.max(1, Number(arg.split('=')[1]) || 1);
...
function usageExit(code = 0) {
  console.log(`Usage: node wav2mp3-folder.mjs [rootDir] [--bitrate=192k] [--delete-original] [--overwrite] [--concurrency=4] [--dry-run] [--verbose]`);
  process.exit(code);
}
...
const LOG = (...xs) => { if (opts.verbose) console.log(...xs); };
const INFO = (...xs) => console.log(...xs);

async function which(cmd) {
  const paths = (process.env.PATH || '').split(path.delimiter);
  const exts = process.platform === 'win32' && process.env.PATHEXT ? process.env.PATHEXT.split(';') : [''];
...
function runProcess(cmd, args, optsSpawn = {}) {
  return new Promise((resolve, reject) => {
    const ps = spawn(cmd, args, { stdio: ['ignore', 'pipe', 'pipe'], ...optsSpawn });
    let stdout = '', stderr = '';
...
async function writeJson(filePath, obj) {
  const data = JSON.stringify(obj, null, 2) + os.EOL;
...
async function convertFile(converter, inputPath, outputPath, bitrate, { overwrite = false, dryRun = false } = {}) {
  if (!overwrite) {
  ...
  if (converter.name === 'ffmpeg') {
    ...
    const args = ['-hide_banner', '-loglevel', 'error', '-y', '-i', inputPath, '-codec:a', 'libmp3lame', '-b:a', bitrate, outputPath];
...
async function processVersion(converter, rootDir, version, options) {
  ...
  for (const [key, fname] of Object.entries(mapping)) {
    if (typeof fname === 'string' && fname.toLowerCase().endsWith('.wav')) {
      const inputPath = path.join(versionDir, fname);
      const outName = toMp3Name(fname);
      const outputPath = path.join(versionDir, outName);
  ...
  async function worker() {
    while (true) {
      ...
      try {
        const conversion = await convertFile(converter, task.inputPath, task.outputPath, options.bitrate, { overwrite: options.overwrite, dryRun: options.dryRun });
        if (conversion.converted || conversion.dry) {
          convertedCount++;
          results.push({ task, ok: true });
          // Optionally delete original
          if (options.deleteOriginal && !options.dryRun && conversion.converted) {
            try {
  ...
  const succeededOutNames = new Set(results.filter(r => r.ok).map(r => r.task.fname)); // original wav names that succeeded
  if (!options.dryRun) {
    // Backup index.json
    ...
    for (const r of results) {
      if (r.ok) {
        ...
        if (mapping[key] === fname) {
          ...
          LOG(`Skipping update for ${key} because mapping changed (was ${mapping[key]})`);
        }
      }
    }
    if (changed) {
  ...
  return { version, converted: convertedCount, skipped: skippedCount, total: tasks.length };
}

async function main() {
  const converter = await detectConverter();
  if (!converter) {
    console.error('No converter found. Please install ffmpeg (recommended) or lame and ensure it is in your PATH.');
  ...
  const topIndexPath = path.join(rootDir, 'index.json');
  let versions = [];
  ...
  if (versions.length === 0) {
    INFO('No "versions" in top index.json; searching for subfolders with index.json');
    const dirents = await fs.readdir(rootDir, { withFileTypes: true });
    for (const d of dirents) {
      if (d.isDirectory()) {
        const candidate = path.join(rootDir, d.name, 'index.json');
  ...
  if (versions.length === 0) {
    console.error('No versions found to process. Ensure a top-level index.json with "versions" or subfolders with index.json exist.');
    process.exit(3);
  }
  ...
  // Summary
  let totalConverted = 0, totalSkipped = 0, totalTasks = 0;
  for (const r of results) {
  ...
  if (opts.dryRun) INFO('Note: dry-run was enabled; no files were actually written or deleted.');
