...
export default function BRecorder({
  packedSampler: brecorderSampler,
  label,
  toggle,
  callbacks,
  offReady,
  ...
  useEffect(() => {
    setVolume(brecorderSampler.getVolume());
    setVibrato(brecorderSampler.getVibrato());
    setAlternatives(brecorderSampler.getAlternatives());
    setVersion(brecorderSampler.getVersion());
  }, [brecorderSampler]);

  const handleVolumeChanged = useCallback(
    (value) => {
      setVolume(value);
      brecorderSampler.setVolume(value);
    },
    [brecorderSampler],
  );
  ...
  const handleVibratoChanged = useCallback(
    (value) => {
      setVibrato(value);
      brecorderSampler.setVibrato(value);
    },
    [brecorderSampler],
  );
  ...
  const handleVersionChanged = useCallback(
    (value) => {
      ...
      setVersion(value);
      brecorderSampler.setVersion(value, onSamplerChanged);
    },
    [offReady, callbacks, brecorderSampler, onSamplerChanged],
  );
          ...
          <div className="flex flex-col gap-2 max-w-full sm:max-w-100">
            ...
            <div className="grid grid-cols-[max-content_1fr] items-center gap-x-4 gap-y-2">
              ...
              <div className="flex items-center gap-1 whitespace-nowrap">
                <label>{t("brecorder.volume")}:</label>
                <SettingTooltip>{t("brecorder.tips.volume")}</SettingTooltip>
              </div>
              ...
              {/* Vibrato */}
              <div className="flex items-center gap-1 whitespace-nowrap">
                <label>{t("brecorder.vibrato")}:</label>
                <SettingTooltip>{t("brecorder.tips.vibrato")}</SettingTooltip>
              </div>
              <div className="min-w-0">
                <DuoSlideBar
                  min={0}
              ...
              {/* Variant */}
              <div className="flex items-center gap-1 whitespace-nowrap">
                <label>{t("brecorder.variant")}:</label>
                <SettingTooltip>{t("brecorder.tips.variant")}</SettingTooltip>
              </div>
              <div className="min-w-0">
                <DuoSelect
